...
class MotoristaControllerTest {
    ...
    @Test
    @DisplayName("Não retorna lista de motoristas simples e retorna o status 204")
    void retornaSimplesComFalha(){

        when(repository.getMotoristasSimples()).thenReturn(new ArrayList<>());

        ResponseEntity<List<MotoristaSimplesResponse>> listaMotoristas =
    ...
    void retornaSimplesComSucesso(){
        ...
        when(repository.getMotoristasSimples()).thenReturn(List.of(
                new MotoristaSimplesResponse(1, null),
                new MotoristaSimplesResponse(2, null)
        ));
