...
public class PassageiroController {
    ...
    public ResponseEntity<List<PassageiroSimplesResponse>> getSimples() {
        List<PassageiroSimplesResponse> passageirosSimples =
                repository.getPassageirosSimples();
        if (passageirosSimples.isEmpty()){
            return ResponseEntity.status(204).build();
        }
        return ResponseEntity.status(200).body(passageirosSimples);
    }
}
